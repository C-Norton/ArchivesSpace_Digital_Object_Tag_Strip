...
def setup():
    ...
    if not os.path.isfile(logFile):
        if not os.path.exists(logFile):
            ...
            print(f"Cannot write to {logFile}: Is a directory. ArchivesSpace has not been modified")
            return
    else:
...
def strip_tags(session, reponum, resourcenum):
    ...
    while (page <= maxpage):
        page += 1
        result = session.get(f"repositories/{reponum}/digital_objects", params={"page": page}).json()
        maxpage = result["last_page"]
        for result in result["results"]:
            if resourcenum == 0 or {"ref": f"/repositories/{reponum}/resources/{resourcenum}"} in result["collection"]:
...
def process_digital_object(session, result: json):
    ...
    for versionnum in range(len(result["file_versions"])):
        preservicalink = re.findall('<a href="(https://uorrcl.access.preservica.com/.+?/)">',
    ...
    if wasChanged:
        ...
        log(result["uri"], result["file_versions"][1]["file_uri"], edited["file_versions"][1]["file_uri"],
            request.json()["status"])
